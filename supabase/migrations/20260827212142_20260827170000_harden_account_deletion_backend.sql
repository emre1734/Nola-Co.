/*
# Harden Account Deletion Backend — Three Fixes

## Issue 1: Remove unverified review/rating recomputation

The previous implementation invented a rating/total_reviews recomputation
formula (AVG + ROUND + COUNT) for surviving provider_profiles after
customer review CASCADE deletion. No authoritative rating formula was
verified — the reviews table is empty, no trigger/RPC/source writes
provider_profiles.rating or total_reviews. The original contract required
STOP if the authoritative calculation cannot be verified.

This migration REMOVES the invented recomputation. Reviews simply
CASCADE delete per existing FKs. No provider_profiles.rating or
total_reviews is modified during account deletion.

## Issue 2: (No DB change — Edge Function fix only)

## Issue 3: Profile missing + manifest missing must fail closed

The previous implementation created an empty manifest when the profile
was already gone but no manifest existed. This is unsafe because
cross-user job-image prefixes may no longer be reconstructable after
profile/FK detachment.

This migration changes the behavior to FAIL CLOSED: return a
deletion_state_inconsistent error instead of creating a manifest.
The Edge Function translates this to a non-sensitive public response.
...
- No changes to get_account_deletion_eligibility or any other RPC
