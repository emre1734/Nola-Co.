/*
# Fix schedule conflict filter in find_eligible_providers

## Root Cause
The schedule-conflict NOT EXISTS check compared only booking_time windows
without requiring the existing booking to be on the same booking_date as
the new booking. Old accepted bookings from different dates at overlapping
times-of-day incorrectly blocked providers.

## Fix
Add explicit same-date condition: ab.booking_date = v_booking.booking_date
to the schedule conflict NOT EXISTS subquery.

## Blocking statuses
Booking status enum: waiting, accepted, rejected, cancelled, expired.
Only 'accepted' reserves a time slot. No 'completed' or 'confirmed'
booking status exists in the schema.

## Time overlap
Preserved: existing_start < new_end AND existing_end > new_start
Back-to-back (existing_end == new_start) is NOT a conflict.
*/
