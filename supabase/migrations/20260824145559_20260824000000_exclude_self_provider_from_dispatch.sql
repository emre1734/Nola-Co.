/*
# Exclude booking creator's own provider profile from dispatch

## Purpose
Prevents a customer who also has a provider profile from receiving a
booking offer for their own booking. The provider candidate function
find_eligible_providers now excludes the provider profile whose
provider_profiles.profile_id equals the booking's customer_id.

## Changes
1. The booking record loaded into v_booking now also selects b.customer_id
   (previously not loaded).
2. A single predicate is added to the candidate WHERE clause:
     AND pp.profile_id <> v_booking.customer_id

## Preserved
- Function signature, return type, SECURITY DEFINER, search_path, language
- All existing eligibility predicates (role, active, available, verified,
  working days/hours, active-job, schedule-conflict, rejection, prior-offer)
- Haversine distance, 5 km radius, ranking, LIMIT 20
- No changes to accept_booking_offer, dispatch_booking_wave_one, RLS, schema,
  GPS, tracking, or application code
*/
...
BEGIN
  ...
  SELECT b.status, b.provider_id, b.latitude, b.longitude,
         b.booking_date, b.booking_time, b.service_id, b.customer_id
  INTO v_booking
  ...
  WITH eligible AS (
    ...
    WHERE p.role = 'provider'
      ...
      -- Exclude the booking creator's own provider profile
      AND pp.profile_id <> v_booking.customer_id
